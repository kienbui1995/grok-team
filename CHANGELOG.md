...
### Added
- **Story gates rail (Phase 1 / stack B)**: workbench chrome toggles a G1–G5 overlay (Spec / ADR / Dev diff / QC / Demo non-prod) on the existing project–session–diff surface. Lean sample paths come from `artifacts/engineering/grok-team-lean-poc` (overridable via same-origin `/artifacts/…` JSON). Unofficial MIT RongleCat footer; no third route, factory dashboard, or ship-prod CTA.
...
**中文 · 新增**
- **Story gates 轨道（Phase 1 / stack B）**：在现有工作台（项目 / 会话 / diff）用顶栏开关打开 G1–G5 浮层，不新增路由。Lean 样例路径读 `artifacts/engineering/grok-team-lean-poc`（可用同源 `/artifacts/…` JSON 覆盖）。文案为 Demo non-prod；页脚 unofficial + MIT RongleCat；无工厂看板、无正式上线 CTA。
