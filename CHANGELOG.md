...
### Added
- **Software Works × Laya triage (opt-in sidecar)**: Settings → Extensions → Agents can enable a local System 1 sidecar (`pip install laya>=0.3.3` on the user’s machine; this app never pip-installs it). Studio can **Suggest** priority, start-delivery template, and first role with calibrated confidence. Apply is a click. Ship-ready is a status line only — it never unlocks Ship, never writes `~/.grok`, never invents Host ids. Router is required (state may be Vietnamese). Default **off**. Not an oracle; Grok Build stays the only agent.
- **Software Works board drag & drop**: SDLC Studio cards drag between Backlog → Design → Build → Review → Ship columns. The hovered column highlights; a gated Ship column shows a red no-drop tone. Dropping reuses the same stage path as the context menu — same-stage drops are no-ops, and Ship without delivery Reviewer + QA notes refuses with the same lock status and writes nothing. The context menu remains the accessible path.
...
**中文 · 新增**
- **Software Works × Laya 分诊（可选侧车）**：设置 → 扩展 → 代理可开启本机 System 1 侧车（用户自行 `pip install laya>=0.3.3`，应用不会代装）。工坊可**建议**优先级、开始交付模板和起始角色，并带校准置信度。应用建议需点击。发布就绪只是状态行 — 不解锁发布、不写 `~/.grok`、不编造 Host id。必须走 Router（状态可能是越南语）。默认关。不是神谕；Grok Build 仍是唯一智能体。
- **Software Works 看板拖拽**：SDLC Studio 卡片可在 待办→设计→构建→评审→发布 列间拖拽，悬停列高亮；被门槛锁住的发布列显示红色不可放置提示。落点复用与右键菜单相同的阶段路径 — 同列拖放为空操作，评审/测试备注不齐时拖到发布会被拒绝并给出同样的锁定提示，不写任何内容。右键菜单仍是无障碍路径。
